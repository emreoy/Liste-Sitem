...
class qa_html_theme extends qa_html_theme_base
{
	...
	{
		$this->output(
			'<html lang="en-US" class="king-night">',
	...
	public function q_view($q_view)
	{
		...
		if (!empty($q_view)) {
			...
			$this->output('<div class="king-nextprev">');
			$this->get_next_q();
			$this->get_prev_q();
			$this->output('</div>');
			$this->output('</DIV>');
	...
	{
		$this->output('<div class="king-search-container">');
		$this->search();
		$this->output('</div>');
	}
	...
	public function header()
	{
		...
		$this->output('<DIV CLASS="king-header-top">');
		...
		$this->output('<div class="king-left-toggle" data-toggle="dropdown" data-target=".leftmenu" aria-expanded="true" role="button"><span class="left-toggle-line"></span></div>');

		$this->output('<div class="kingadd" data-toggle="dropdown" data-target=".king-submit" aria-expanded="false" role="button"><i class="fas fa-feather-alt"></i></div>');
		$this->nav_main_sub();
		$this->output('</div>');
	...
	public function kingsubmit()
	{
		if (!qa_opt('disable_image') || !qa_opt('disable_video') || !qa_opt('disable_news') || !qa_opt('disable_poll') || !qa_opt('disable_list')) {
			$this->output('<div class="king-submit open">');
	...
	public function header_right()
	{
		...
		if (!qa_is_logged_in()) {
			$this->output('<li>');
			$this->output('<a class="reglink" href="./register"><i class="fas fa-globe-europe fa-lg"></i></a>');
			$this->output('</li>');
			$this->output('<li>');
			$this->output('<div class="reglink" data-toggle="modal" data-target="#loginmodal" role="button" title="Login"><i class="far fa-user fa-lg"></i></div>');
			$this->output('</li>');
		} else {
			$this->userpanel();
		}
		...
		if (qa_using_categories()) {
			$this->output('<li class="cats-button"><span data-toggle="dropdown" data-target=".king-cat-main" aria-expanded="false" class="cats-toggle"><i class="far fa-compass fa-lg"></i></span></li>');
		}
		$this->kingnight();
	...
	public function kingnight()
	{
		
		$this->output('<div class="kingnight">');
		$this->output('<input type="checkbox" id="king-night" name="king-night" class="hide" value="Yes" />');
		$this->output('<label for="king-night" class="king-night-box">');
		$this->output('<i class="fas fa-moon"></i>');
		$this->output('</label>');
		$this->output('</div>');	
	...
	public function userpanel2()
	{
		if (!qa_is_logged_in()) {
			...
			$this->output('<div class="king-modal-header"><button type="button" class="king-modal-close" data-dismiss="modal" aria-label="Close"><i class="icon fa fa-fw fa-times"></i></button><h4 class="modal-title">GİRİŞ</h4></div>');
			...
			$this->output('<form action="' . qa_path_html('login') . '" method="post">
				...
				<input type="submit" value="Giriş Yap" id="king-login" name="dologin" />
				</form>');
			$this->output('</div>');
	...
	public function q_item_main($q_item)
	{
		...
		$this->q_item_content($q_item);
		$this->output('<div class="yoriz" style="
    position: absolute;
    bottom: 0;
    z-index: 4;
">');
	...
	public function get_prev_q()
	{
		...
		while ($prev_link = qa_db_read_one_assoc($prev_q, true)) {
			...
			$this->output('<A HREF="' . qa_q_path_html($pid, $title, $q_item) . '" CLASS="king-prev-q"><span class="next-title"><i class="fas fa-arrow-right"><span class="next-back" style=background-image:url(' . $q_item . ')></span></i></span>' . $title . ' </A>');
		}

	}
	public function darktheme()
	{
		$this->output('<div class="dark_mode">
...
header('Location: index.php');
	...
	public function get_next_q()
	{
		...
		while ($next_link = qa_db_read_one_assoc($next_q, true)) {
			...
			$this->output('<A HREF="' . qa_q_path_html($pid, $title) . '" CLASS="king-next-q"><span class="next-title"><i class="fas fa-arrow-left"></i></span> ' . $title . '</A>');
